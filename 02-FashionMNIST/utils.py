...
def test_augmentations(image, transforms, n_augmentations=5):
    img_pil = PIL.Image.fromarray(image)
    fig = figure()
    for i in range(n_augmentations):
        fig.add_subplot(1, n_augmentations, i + 1)
        img_transformed = transforms['train'](img_pil)

        # Convert the tensor to a format suitable for imshow
        img_np = img_transformed.numpy().transpose((1, 2, 0))
        img_np = img_np.squeeze()  # Remove the channel dimension
